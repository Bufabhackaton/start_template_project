...
const VALIDATOR_PATH_CANDIDATES = [
  ...
  // Default vendored layout: <workspace>/bufab-mcp/scripts/validate.mjs
...
export function formatViolationReport(displayPath, result) {
  ...
  lines.push("");
  lines.push("Fix the file and re-edit. Full rules: call ui_export_markdown via bufab-mcp.");
  return lines.join("\n");
}
...
function buildReminderFromGuidelines(guidelines) {
  ...
  lines.push("");
  lines.push("Full reference: call ui_export_markdown via bufab-mcp");
  return lines.join("\n");
}
